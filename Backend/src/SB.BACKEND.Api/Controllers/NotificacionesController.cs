...
public sealed class NotificacionesController(INotificationApplicationService service)
    : ControllerBase
{
    [HttpGet]
    public Task<IReadOnlyCollection<NotificacionResponse>> Get(CancellationToken ct)
    {
        return service.GetMineAsync(ct);
    }

    [HttpGet("no-leidas/count")]
    public async Task<object> Count(CancellationToken ct)
    {
        return new { count = await service.UnreadCountAsync(ct) };
    }
