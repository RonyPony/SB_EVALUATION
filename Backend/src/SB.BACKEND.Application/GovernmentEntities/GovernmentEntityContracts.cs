...
{
    Task<PagedResult<GovernmentEntityResponse>> GetAllAsync(
        GovernmentEntityListQuery query,
        CancellationToken ct
    );
    Task<PagedResult<GovernmentEntityResponse>> GetDeletedAsync(
        GovernmentEntityListQuery query,
        CancellationToken ct
    );
    ...
    Task RestoreAsync(Guid id, string version, CancellationToken ct);
}

public interface IGovernmentEntityRepository
{
    Task<(IReadOnlyCollection<EntidadGubernamental> Items, int Total)> GetPagedAsync(
        GovernmentEntityListQuery query,
        bool deletedOnly,
        CancellationToken ct
    );
    ...
    void SetOriginalRowVersion(EntidadGubernamental entity, byte[] version);
}
