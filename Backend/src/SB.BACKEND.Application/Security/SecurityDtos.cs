...
{
    [Required, StringLength(50, MinimumLength = 3)] public string Username { get; init; } = string.Empty;
...
{
    [Required, StringLength(50, MinimumLength = 2)] public string Name { get; init; } = string.Empty;
...
public sealed record UserResponse(Guid Id, string Username, string Email, bool IsActive, DateTimeOffset CreatedAt, IReadOnlyCollection<string> Roles);
