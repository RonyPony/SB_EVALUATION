...
public sealed class ChangeAreaStatusRequest
{
...
public sealed class CreateSolicitudRequest
{
    ...
    public TipoSolicitud Tipo { get; init; }

    [EnumDataType(typeof(PrioridadSolicitud))]
    public PrioridadSolicitud Prioridad { get; init; } = PrioridadSolicitud.Media;
    public Guid AreaId { get; init; }

    [StringLength(1000)]
    public string? ReferenciaEvidencia { get; init; }
    public DateTimeOffset? FechaCompromiso { get; init; }
}
...
public sealed class AssignSolicitudRequest
{
...
public sealed class ChangeSolicitudStatusRequest
{
    ...
    [StringLength(2000)]
    public string? ComentarioResolucion { get; init; }

    [Required]
    public string RowVersion { get; init; } = "";
}

public sealed class ReopenSolicitudRequest
{
    [Required, StringLength(1000)]
    public string Comentario { get; init; } = "";
...
public sealed class SolicitudListQuery
{
    ...
    public Guid? ResponsableId { get; init; }
    public bool? SinResponsable { get; init; }
    public TipoSolicitud? Tipo { get; init; }
    public DateTimeOffset? CreadaDesde { get; init; }
    public DateTimeOffset? CreadaHasta { get; init; }
    public DateTimeOffset? CompromisoDesde { get; init; }
    public DateTimeOffset? CompromisoHasta { get; init; }
...
public sealed record DashboardResponse(
    ...
    int Cerradas,
    int Vencidas,
    IReadOnlyDictionary<EstadoSolicitud, int> PorEstado,
    IReadOnlyDictionary<PrioridadSolicitud, int> PorPrioridad,
    IReadOnlyCollection<DashboardItem> Ultimas
);
