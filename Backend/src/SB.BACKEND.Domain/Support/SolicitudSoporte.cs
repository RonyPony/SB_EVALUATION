...
{
    private static readonly IReadOnlyDictionary<EstadoSolicitud, EstadoSolicitud[]> Transitions =
    ...
    )
    {
        ...
        ReferenciaEvidencia = evidencia;
        FechaCompromiso = compromiso?.ToUniversalTime();
    }
    ...
    public ICollection<HistorialSolicitud> Historial { get; private set; } = [];
    public ICollection<ComentarioSolicitud> Comentarios { get; private set; } = [];
    ...
    {
        if (Estado == EstadoSolicitud.Cerrada || IsDeleted)
    ...
    public void CambiarPrioridad(PrioridadSolicitud prioridad)
    {
        if (IsDeleted || Estado == EstadoSolicitud.Cerrada)
            throw new InvalidOperationException();
        Prioridad = prioridad;
    }

    public Guid? Asignar(Guid responsableId, Guid usuarioId, DateTimeOffset now)
    {
        if (IsDeleted || Estado == EstadoSolicitud.Cerrada)
            throw new InvalidOperationException();
        var previous = ResponsableId;
        ResponsableId = responsableId;
        FechaAsignacion = now.ToUniversalTime();
        Historial.Add(
            new HistorialSolicitud(
                Id,
                Estado,
                Estado,
                $"Asignación: {previous?.ToString() ?? "Sin responsable"} -> {responsableId}",
                usuarioId
            )
        );
        return previous;
    ...
    public EstadoSolicitud CambiarEstado(
    ...
    {
        if (IsDeleted || !Transitions[Estado].Contains(nuevo))
            throw new InvalidOperationException();
        if (nuevo == EstadoSolicitud.Cerrada && string.IsNullOrWhiteSpace(ComentarioResolucion))
            throw new InvalidOperationException();
        var anterior = Estado;
        Estado = nuevo;
        if (nuevo == EstadoSolicitud.EnProgreso && FechaInicioAtencion is null)
            FechaInicioAtencion = now.ToUniversalTime();
        if (nuevo == EstadoSolicitud.Resuelta)
            FechaResolucion = now.ToUniversalTime();
        if (nuevo == EstadoSolicitud.Cerrada)
            FechaCierre = now.ToUniversalTime();
        Historial.Add(new HistorialSolicitud(Id, anterior, nuevo, comentario, usuarioId));
        return anterior;
    }

    public void RegistrarResolucion(string comentario)
    {
    ...
    public void Reabrir(string comentario, Guid usuarioId, DateTimeOffset now)
    {
        ...
        FechaCierre = null;
        Historial.Add(new HistorialSolicitud(Id, anterior, Estado, comentario, usuarioId));
    }
