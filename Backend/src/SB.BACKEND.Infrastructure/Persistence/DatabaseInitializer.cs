...
internal sealed class DatabaseInitializer(
    ...
    IOptions<DemoUserSettings> demoOptions,
    IGovernmentEntitySeeder governmentSeeder
) : IDatabaseInitializer
{
    public async Task InitializeAsync(CancellationToken ct = default)
    {
        ...
        var administrador = await GetOrCreateRoleAsync(
            "Administrador",
            "Administración integral de la plataforma de soporte.",
            ct
        );
        await GetOrCreateRoleAsync("Analista", "Atención y gestión de solicitudes asignadas.", ct);
        await GetOrCreateRoleAsync(
            "Solicitante",
            "Registro y seguimiento de solicitudes propias.",
            ct
        );
        var permissions = await dbContext.Permissions.ToListAsync(ct);
        foreach (var permission in permissions)
            if (
                admin.RolePermissions.All(x =>
                {
                    return x.PermissionId != permission.Id;
                })
            )
                admin.RolePermissions.Add(new RolePermission(admin.Id, permission.Id));
        ...
        if (!string.IsNullOrWhiteSpace(demo.Username) && !string.IsNullOrWhiteSpace(demo.Password))
        {
            ...
            {
                var email = string.IsNullOrWhiteSpace(demo.Email)
                ...
                var user = new User(
                    ...
                    passwordHasher.Hash(demo.Password)
                );
                user.UserRoles.Add(new UserRole(user.Id, admin.Id));
                user.UserRoles.Add(new UserRole(user.Id, userRole.Id));
                user.UserRoles.Add(new UserRole(user.Id, administrador.Id));
                dbContext.Users.Add(user);
            }
            else if (
                existingUser.UserRoles.All(x =>
                {
                    return x.RoleId != administrador.Id;
                })
            )
                existingUser.UserRoles.Add(new UserRole(existingUser.Id, administrador.Id));
        }
    ...
    )
    {
        ...
        var role = await dbContext
            .Roles.Include(x => x.RolePermissions)
            .SingleOrDefaultAsync(x => x.NormalizedName == normalized, ct);
        if (role is not null)
            return role;
        role = new Role(name, normalized, description);
