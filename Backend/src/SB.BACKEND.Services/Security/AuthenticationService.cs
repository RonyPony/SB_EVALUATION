...
) : IAuthenticationService
{
    public async Task<UserResponse> RegisterAsync(RegisterUserRequest request, CancellationToken ct)
    {
        ...
        var normalizedUsername = SecurityMappings.Normalize(username);
        var normalizedEmail = SecurityMappings.Normalize(email);
    ...
    public async Task<LoginResponse?> LoginAsync(LoginRequest request, CancellationToken ct)
    {
        ...
        var permissions = user
            .UserRoles.SelectMany(x =>
            {
                return x.Role.RolePermissions;
            })
            .Select(x =>
            {
        ...
        var token = tokens.GenerateToken(
            new AuthenticatedUser(user.Id, user.Username, roles, permissions)
        );
