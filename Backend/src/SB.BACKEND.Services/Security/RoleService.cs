...
{
    public async Task<IReadOnlyCollection<RoleResponse>> GetAllAsync(CancellationToken ct)
    {
        return
        [
            .. (await roles.GetAllAsync(ct)).Select(x =>
            {
                return x.ToResponse();
            }),
        ];
    ...
    public async Task<RoleResponse> CreateAsync(CreateRoleRequest request, CancellationToken ct)
    {
        ...
        var normalized = SecurityMappings.Normalize(name);
        if (await roles.NameExistsAsync(normalized, null, ct))
            throw new ConflictException("The role name already exists.");
        var role = new Role(name, normalized, request.Description?.Trim());
        roles.Add(role);
    ...
    )
    {
        ...
        var name = request.Name.Trim();
        var normalized = SecurityMappings.Normalize(name);
        if (await roles.NameExistsAsync(normalized, id, ct))
            throw new ConflictException("The role name already exists.");
        role.Update(name, normalized, request.Description?.Trim());
        await unitOfWork.SaveChangesAsync(ct);
        return role.ToResponse();
    }
    ...
    public async Task AssignPermissionAsync(Guid roleId, Guid permissionId, CancellationToken ct)
    {
        ...
        _ = await GetPermissionAsync(permissionId, ct);
        if (
    ...
    public async Task RemovePermissionAsync(Guid roleId, Guid permissionId, CancellationToken ct)
    {
        var role = await GetRoleAsync(roleId, ct);
        var relation =
            role.RolePermissions.SingleOrDefault(x =>
            {
                return x.PermissionId == permissionId;
            }) ?? throw new NotFoundException("The role does not have this permission.");
        role.RolePermissions.Remove(relation);
        await unitOfWork.SaveChangesAsync(ct);
    }
    ...
    private async Task<Permission> GetPermissionAsync(Guid id, CancellationToken ct)
    {
        return await permissions.GetByIdAsync(id, ct)
